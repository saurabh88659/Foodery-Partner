...
import AllPoductSubCategoryComponent from '../component/AllPoductSubCategoryComponent';

function AllProductSubCategory({navigation, route}) {
  const selectedItems = useSelector(state => state.requiredata.selectedItem);
  console.log('selet item ', selectedItems);
  ...
  const GetAllSubCategoryList = async () => {
    setLoading(true);
    const res = await handleGetAllSubCategoryList(ProductId);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={{paddingBottom: '17%'}}>
        <AllPoductSubCategoryComponent
          ...
          onPress={data => navigation.navigate('AllProductsItem', {data})}
        />
...
export default AllProductSubCategory;
